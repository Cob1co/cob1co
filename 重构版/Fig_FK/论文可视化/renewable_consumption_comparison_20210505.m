% 可再生能源消纳展示脚本 - 5月5日
% 功能：基于原始数据展示优化调度效果（论文格式）！！！英文！！！


function renewable_consumption_simple_20210505()
    %% 数据加载
    ...
    % 设置图例（使用标准英文简称）
    legend([h1, h2, h3, h4, h5], {'Wind', 'PV', 'Total Renewable', 'Before Optimization', 'Optimization Gain'}, ...
           'Location', 'northwest', 'FontSize', 12, 'FontName', 'Times New Roman', ...
           'Box', 'off');
    
    % 添加标题显示消纳率改善
    title(sprintf('Renewable Energy Consumption: %.1f%% → %.1f%%', consumption_rate_before, consumption_rate_after), ...
